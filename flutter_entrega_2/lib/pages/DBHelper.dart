...
class DBHelper {
  ...
  _onCreate(Database db, int version) async {
    //CREACION TABLA
    await db.execute(
        "CREATE TABLE $TABLE ($ID INTEGER PRIMARY KEY, $PATENTE TEXT, $MARCA TEXT, $PRECIO TEXT)");
  }
  ...
  Future<List<Car>> getCars() async {
    ...
    var dbClient = await db;
    List<Map> maps =
        await dbClient.query(TABLE, columns: [ID, PATENTE, MARCA, PRECIO]);
    //List<Map> maps = await dbClient.rawQuery("SELECT * FROM $TABLE");
    List<Car> cars = [];
    if (maps.length > 0) {
      //SI HAY ALGO EN LA LISTA  DE MAPAS, LO RECORRO PARA AGREGARLOS
  ...
  Future<int> update(Car car) async {
    ...
    return await dbClient
        .update(TABLE, car.toMap(), where: '$ID = ?', whereArgs: [car.patente]);
  }
